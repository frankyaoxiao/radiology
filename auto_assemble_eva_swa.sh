...
EXPECTED=(
  "${SUB}/eva02_s0_swa_va.csv"
  "${SUB}/eva02_s1_swa_va.csv"
  "${SUB}/eva02_s2_swa_va.csv"
)

echo "Waiting for 3 EVA-02 SWA CSVs..." >&2
...
eva_swa, ids = pool_mean([f'{SUB11}/eva02_s{s}_swa_va.csv' for s in (0,1,2)])
eva_swa.to_csv(f'{SUB11}/eva02_swa_3seed_mean.csv', index=False, float_format='%.6f')
eva_swa.to_csv(f'{SUB11}/ladder/EVW_swa_alone.csv', index=False, float_format='%.6f')
...
for lab in LABELS:
    if lab == 'Cardiomegaly':
        ...
        ens[lab] = (0.10*om[lab].to_numpy() + 0.10*hp[lab].to_numpy() +
                    ...
                    0.16*sg14[lab].to_numpy() + 0.30*eva_swa[lab].to_numpy())
ens.to_csv(f'{SUB11}/ladder/PP_6leg_evaSWA_30.csv', index=False, float_format='%.6f')
print('wrote EVW_swa_alone + PP_6leg_evaSWA_30')
"

for v in EVW_swa_alone PP_6leg_evaSWA_30; do
